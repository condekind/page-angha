...
export class BenchmarksComponent implements OnInit {
  ...
  chartGridMaker = (meta) => {
    let charts = [''].concat(meta.chartUrlArray).reduce((ch, url) =>
          ch + '<img src="' + url + '" alt="" class="chart-img">')
    return `<div class="img-grid">${charts}</div>`
  }

  extIconLink = (url, iconName) => {
    return `<a href="${url}" target="_blank" rel="noreferrer noopener"><mat-icon>${iconName}</mat-icon></a>`
  }
